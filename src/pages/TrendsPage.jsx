  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        <Skel width={220} height={24} style={{ marginBottom: 8 }} />
        <Skel width={280} height={13} />
      </div>
      {/* Filter bar */}
      <Skel height={80} radius={10} style={{ marginBottom: 20 }} />
      {/* Summary cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
        {[...Array(4)].map((_, i) => (
          <div key={i} style={{ background: C.navy, border: `1px solid ${C.border}`, borderRadius: 10, padding: '14px 18px', borderLeft: `3px solid ${C.border}` }}>
            <Skel width={28} height={24} style={{ marginBottom: 6 }} />
...
export default function TrendsPage() {
  ...
  if (!data?.success) {
    return <p style={{ color: C.muted, padding: 20 }}>{data?.message || 'No data'}</p>
  }
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              <Pill label="Price Hurting" icon="bi bi-tag-fill"           active={flagFilter === 'priceHurting'} onClick={() => setFlagFilter('priceHurting')} color={C.pink}   />
              <Pill label="Losing Share"  icon="bi bi-exclamation-triangle" active={flagFilter === 'losingShare'}  onClick={() => setFlagFilter('losingShare')}  color={C.yellow} />
            </div>
          </div>
      ...
      <div className="trends-main" style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 16 }}>
        <div style={{ background: C.navy, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
          ...
          <div style={{ overflowY: 'auto', maxHeight: 600 }}>
            {filteredKeywords.length === 0 ? (
              ...
              <div key={i} onClick={() => setSelected(k)} style={{ padding: '10px 14px', cursor: 'pointer', borderLeft: `3px solid ${selected?.search_query === k.search_query ? C.orange : 'transparent'}`, background: selected?.search_query === k.search_query ? 'rgba(88,166,255,0.07)' : 'transparent', borderBottom: `1px solid rgba(48,54,61,0.6)` }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <div style={{ color: C.white, fontSize: 12, fontWeight: 500 }}>{k.search_query}</div>
                  {selected?.search_query === k.search_query && (
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/keyword/${encodeURIComponent(k.search_query)}?filter=${timeFilter}`) }} style={{ background: 'rgba(88,166,255,0.15)', border: '1px solid rgba(88,166,255,0.3)', borderRadius: 6, padding: '2px 8px', color: '#58A6FF', fontSize: 10, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                      <i className="bi bi-arrow-right" style={{ fontSize: 10 }} /> Deep Dive
                    </button>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <span style={{ color: patternColor(k.volume_trend?.pattern), fontSize: 10 }}>
                    {k.volume_trend?.pattern?.includes('RISING') ? '↑' : k.volume_trend?.pattern?.includes('DECLINING') ? '↓' : '→'} {k.volume_trend?.pattern?.replace(/_/g, ' ')}
                  </span>
                ...
                <div style={{ color: C.subtle, fontSize: 10, marginTop: 2 }}>{k.volume_trend?.latest_volume?.toLocaleString()} vol · {k.weeks_tracked}wk</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ background: C.navy, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
              {[
                ...
                <button key={v.key} onClick={() => setView(v.key)} style={{ padding: '5px 12px', borderRadius: 6, border: `1px solid ${view === v.key ? C.orange : C.border}`, background: view === v.key ? 'rgba(88,166,255,0.12)' : 'transparent', color: view === v.key ? C.orange : C.muted, fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <i className={v.icon} style={{ fontSize: 13, color: view === v.key ? C.orange : C.muted }} /> {v.label}
                </button>
              ))}
            </div>
              ...
              {view === 'volume' && (
                <ResponsiveContainer width="100%" height={190}>
                  <ComposedChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    <defs><linearGradient id="volGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#58A6FF" stopOpacity={0.25} /><stop offset="100%" stopColor="#58A6FF" stopOpacity={0.02} /></linearGradient></defs>
                    ...
                    {chartData.some(d => d.forecast) && <ReferenceLine x="Forecast" stroke={C.purple} strokeDasharray="4 2" label={{ value: '🔮', fill: C.purple, fontSize: 12 }} />}
                  </ComposedChart>
                </ResponsiveContainer>
              )}
              {view === 'share' && (
                <ResponsiveContainer width="100%" height={190}>
                  <ComposedChart data={chartData.filter(d => d.share != null)} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    <defs><linearGradient id="shareGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3FB950" stopOpacity={0.3} /><stop offset="100%" stopColor="#3FB950" stopOpacity={0.02} /></linearGradient></defs>
                    <CartesianGrid stroke={C.border} strokeDasharray="3 3" vertical={false} opacity={0.5} />
                    <XAxis dataKey="week" tick={{ fill: C.muted, fontSize: 11 }} axisLine={false} tickLine={false} />
            ...
            <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
              {[
            ...
            {selected.price_analysis?.price_hurting_share && (
              <div style={{ background: 'rgba(248,81,73,0.07)', border: '1px solid rgba(248,81,73,0.25)', borderRadius: 8, padding: '12px 14px' }}>
                ...
                <div style={{ color: C.muted, fontSize: 12, lineHeight: 1.5 }}>Gap widened from <strong style={{ color: C.white }}>{selected.price_analysis.first_week_gap_pct}%</strong> to 
                <strong style={{ color: C.red }}>{selected.price_analysis.latest_gap_pct}%</strong> · Correlation: <strong style={{ color: C.red }}>{selected.price_analysis.correlation}</strong> ({selected.price_analysis.correlation_strength})</div>
              </div>
            )}
          </div>
        ) : (
